    ...
    switch (mode) {
      case "new":
        if (consigner.name === "") {
          ...
          if (confirm("등록하시겠습니까?")) {
            axios
              .post("api/consignProduct", product)
              .then((res) => {
                if (res.data) {
                  alert("등록 되었습니다.\n⊳ ID : " + res.data);
                  onPreviousStep();
            ...
            axios.put("api/customer", consigner).catch((error) => {
              alert("서버에러");
              console.log(error);
            });
          }
      ...
      case "old":
        if (confirm("수정하시겠습니까?")) {
          axios
            .put("api/consignProduct", product)
            .then((res) => {
              alert("수정되었습니다.");
              setIsSearched(false);
            })
        ...
        {mode === "new" && !consigner.boolConsign && (
          <>
            <Paper variant="outlined" className={classes.item}>
              <Typography variant="h6" align="center" color="primary">
                PASS YOUR COLLECTION
              </Typography>
              <Typography variant="h6" align="center" color="primary" paragraph>
                이야기가 있는 마켓발견 맡겨팔기 신청서
              </Typography>
              <p>
                1. 맡겨팔기 가능한 물품: 판매가 5만원 이상/ 의류,잡화 경우
                모조품 (이미테이션)은 불가능 함{" "}
              </p>
              <p>
                2. 맡겨팔기 신청서 작성 시 제품관련 참고사항을 작성해 주시면
                판매 감정에 도움 됩니다.
              </p>
              <p>
                3. 맡겨팔기신청 후 3개월 이내 판매되지 않는 상품은 개별연락을
                통해 맡겨팔기 연장여부를 결정하며 개별 연락 후 1개월 내 연락이
                닿지 않는 경우 제품의 소유권은 '마켓발견'에 있습니다 .
              </p>
              <p>4. 개인정보 수집 및 이용 동의 거부</p>
              <p>
                5. 맡겨팔기기간 연장 경우, 기간에 따라 보관특성상 물품 변형이
                생길 수 있습니다.{" "}
              </p>
              <p>
                6. 마켓발견 카톡으로 사진을 보내주시면 "마켓발견"에서
                맡겨팔기가능 여부를 연락드리며 가격확정시 맡겨팔기가 시작됩니다.{" "}
              </p>
              <p>
                7. 수수료는 40% 로 판매시 판매금액의 65%를 마켓발견 포인트로
                적립해드립니다. 입금 요청시에는 요청일에 판매금액의 60%가 가까운
                25일에 지급됩니다. 본 위탁자는 위의 사항에 동의하였으며
                맡겨팔기업무를 마켓발견에 의뢰합니다.{" "}
              </p>
            </Paper>
            <Grid item xs={12}>
              <FormControlLabel
                control={
                  <Checkbox className={classes.checkbox} onChange={onAgree} />
                }
      ...
      {(mode === "new" || isSearched) && (
        <Grid item xs={12}>
          ...
          <Paper variant="outlined" className={classes.item}>
            ...
            {mode === "new" && (
              <Grid container justify="flex-end">
                <Button className={classes.next} onClick={onPreviousStep}>
                  이전
                </Button>
                <Button className={classes.next} onClick={onReset}>
                  초기화
                </Button>
              </Grid>
            )}
