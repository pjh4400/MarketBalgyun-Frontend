...
import { Container, Paper, Typography, Grid, Button, TextField, InputAdornment } from "@material-ui/core";
...
const Trader = ({ history }) => {
    ...
    const onSubmitTraderItem = (e) => {
        ...
        switch (mode) {
            case 'new':
                if (confirm("등록하시겠습니까?")) {
                    axios.post("api/trader", traderItem)
                        .then((res) => {
            ...
            >
                ...
                {mode !== "old" &&
                    <form className={classes.form} onSubmit={onSubmitTraderItem}>
                        ...
                        {mode === 'new'
                            ?
                            <Button className={classes.submit} size="large" type="submit">매입처 등록</Button>
                            :
                            <Grid container spacing={2}>
                                <Grid item xs={12} sm={6}>
